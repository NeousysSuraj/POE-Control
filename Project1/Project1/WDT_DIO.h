//*****************************************************************************
//  Copyright (c) 2015, Neousys Technology Inc. All rights reserved.
//
//  Library:     WDT_DIO
//  Revision:    2.1.0
...
//  Remark 3:    These libraries do not support WOW64.
//*****************************************************************************
...
#define CAN_MSG_USE_DIR_FILTER  (0x00000010 | CAN_MSG_USE_ID_FILTER)
#define CAN_MSG_USE_EXT_FILTER  (0x00000020 | CAN_MSG_USE_ID_FILTER)
...
BOOL __cdecl StartDTIO(void);  // [NOTE] If DICOS is started, this will stop it by internal invocation of StopDICOS
BOOL __cdecl StopDTIO(void);
BOOL __cdecl SetUnitDTIO(WORD unit, int delta);  // unit: DTIO timing unit in micro-second (25~2500 us), delta: fine-tuning timing unit, each +/- is 0.04us
...
BOOL __cdecl StartDTFO(void);  // [NOTE] If DICOS is started, this will stop it by internal invocation of StopDICOS
BOOL __cdecl StopDTFO(void);
BOOL __cdecl SetUnitDTFO(WORD unit, int delta);  // unit: micro-second (25~2500 us), delta: fine-tuning timing unit, each +/- is 0.04us
WORD __cdecl GetUnitDTFO(void);  // current DTFO timing unit in micro-second

// Light-Emitting Diode (LED)
//
BOOL __cdecl LED_SetCurrentDriving(DWORD mode, DWORD current);  // mode: 0)disabled, 1)cc, 2)cv, current:
...
// PoE ports on the PCI card (PCI_POE)
//
BYTE __cdecl PCI_POE_GetStatus(DWORD boardId, DWORD port);  // 0: disabled, 1: enabled
BOOL __cdecl PCI_POE_Enable(DWORD boardId, DWORD port);
BOOL __cdecl PCI_POE_Disable(DWORD boardId, DWORD port);
